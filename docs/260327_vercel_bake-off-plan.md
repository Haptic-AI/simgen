...
| Backend | FastAPI on Azure H100 (`20.69.105.30:8000`) | 4 uvicorn workers, EGL rendering |
...
# H100 server
scp -r backend/ azureuser@20.69.105.30:~/simgen/
ssh azureuser@20.69.105.30 "cd ~/simgen/backend && chmod +x start.sh && ./start.sh"

# Frontend (local or Vercel)
npm install
GOOGLE_API_KEY=... MUJOCO_SERVER_URL=http://20.69.105.30:8000 npm run dev
```
